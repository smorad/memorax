...
def semigroup_scan(
    ...
    Executes a semigroup scan. The semigroup_op MUST be associative, i.e.,
        .. math::

            f(a, f(b,c)) = f(f(a,b), c)

    See https://en.wikipedia.org/wiki/Semigrou for information.
    ...
    new_state = jax.lax.associative_scan(
        fn=eqx.filter_vmap(semigroup_op), elems=scan_inputs, axis=axis
    )
